...
import { JobStatus, QueueDriverName, QUEUE_CONFIG, TAB_COORDINATOR, EVENT_EMITTER, QueueEvent, QUEUE_MANAGER, DEFAULT_QUEUE_CONNECTION_TOKEN } from '@stackra/contracts';
import { Str, MultipleInstanceManager } from '@stackra/ts-support';
import { Logger } from '@stackra/ts-logger';
import { getMetadata, defineMetadata, updateMetadata } from '@vivtel/metadata';
...
var MemoryConnection = class extends BaseConnection {
  ...
  async push(name, data, options) {
    ...
    if (job.uniqueId) {
      for (const existing of this.jobs.values()) {
        if (existing.uniqueId === job.uniqueId && existing.status !== JobStatus.Completed && existing.status !== JobStatus.Failed) {
          return existing.id;
        }
      }
    }
  ...
  */
  async pendingSize(queue2 = "default") {
    return this.countByStatus(queue2, JobStatus.Pending);
  }
  /**
  ...
  */
  async delayedSize(queue2 = "default") {
    return this.countByStatus(queue2, JobStatus.Delayed);
  }
  /**
  ...
  */
  async reservedSize(queue2 = "default") {
    return this.countByStatus(queue2, JobStatus.Reserved);
  }
  /**
...
var LocalStorageConnection = class extends BaseConnection {
  ...
  */
  async push(jobName, data, options) {
    ...
    if (job.uniqueId) {
      const existing = blob.jobs.find((j) => j.uniqueId === job.uniqueId && j.status !== JobStatus.Completed && j.status !== JobStatus.Failed);
  ...
  */
  async pop(queue2 = "default") {
    ...
    for (const job of blob.jobs) {
      const isEligible = (job.status === JobStatus.Pending || job.status === JobStatus.Delayed) && job.availableAt <= now;
  ...
  */
  async size(queue2 = "default") {
    const blob = this.read(queue2);
    return blob.jobs.filter((j) => j.status !== JobStatus.Completed && j.status !== JobStatus.Failed).length;
  }
  /**
  * Number of jobs currently in the Pending state on a queue.
  *
...
var IndexedDBConnection = class extends BaseConnection {
  ...
  async pop(queue2 = "default") {
    ...
    return new Promise((resolve, reject) => {
      ...
      req.onsuccess = () => {
        ...
        const eligible = (job.status === JobStatus.Pending || job.status === JobStatus.Delayed) && job.availableAt <= now;
  ...
  */
  async size(queue2 = "default") {
    return this.countMatching(queue2, (j) => j.status !== JobStatus.Completed && j.status !== JobStatus.Failed);
  }
  /**
  * Number of jobs currently in the Pending state on a queue.
  *
  ...
  */
  async pendingSize(queue2 = "default") {
    return this.countMatching(queue2, (j) => j.status === JobStatus.Pending);
  }
  /**
  * Number of jobs currently in the Delayed state on a queue.
  *
  ...
  */
  async delayedSize(queue2 = "default") {
    return this.countMatching(queue2, (j) => j.status === JobStatus.Delayed);
  }
  /**
  * Number of jobs currently in the Reserved state on a queue.
  *
  ...
  */
  async reservedSize(queue2 = "default") {
    return this.countMatching(queue2, (j) => j.status === JobStatus.Reserved);
  }
  /**
...
var QueueManager = class _QueueManager extends MultipleInstanceManager {
  static {
  ...
  */
  logger = new Logger(_QueueManager.name);
  // ── Lifecycle hooks ────────────────────────────────────────────────────
  /**
...
QueueManager = _ts_decorate([
  Injectable(),
  _ts_param(0, Inject(QUEUE_CONFIG)),
  _ts_param(1, Optional()),
  _ts_param(1, Inject(TAB_COORDINATOR)),
...
QueueEventBus = _ts_decorate2([
  Injectable(),
  _ts_param2(0, Optional()),
  _ts_param2(0, Inject(EVENT_EMITTER)),
...
var ProcessorMetadataAccessor = class {
  static {
  ...
  */
  getProcessorMetadata(target) {
    ...
    return getMetadata(PROCESSOR_METADATA, target);
  }
  /**
  ...
  */
  getOnJobEventMetadata(target) {
    ...
    const meta = getMetadata(ON_JOB_EVENT_METADATA, target);
...
ProcessorMetadataAccessor = _ts_decorate3([
  Injectable()
...
var Worker = class _Worker {
  ...
  */
  logger = new Logger(_Worker.name);
  ...
  */
  async handle(job) {
    this.emit(QueueEvent.JobProcessing, {
      job
    });
    let error;
    let timedOut = false;
    try {
      ...
      this.emit(QueueEvent.JobProcessed, {
        job
      });
    } catch (e) {
      error = e;
      ...
      if (timedOut) {
        this.emit(QueueEvent.JobTimedOut, {
          job,
          error
        });
      }
  ...
  */
  async handleFailure(job, error, timedOut) {
    ...
    this.emit(QueueEvent.JobReleased, {
      job,
      delayMs,
      error
    });
...
var ProcessorSubscribersLoader = class _ProcessorSubscribersLoader {
  static {
  ...
  */
  logger = new Logger(_ProcessorSubscribersLoader.name);
  /**
...
ProcessorSubscribersLoader = _ts_decorate4([
  Injectable(),
  _ts_param3(3, Optional()),
  _ts_param3(3, Inject(EVENT_EMITTER)),
  _ts_metadata3("design:type", Function),
  _ts_metadata3("design:paramtypes", [
    typeof QueueManager === "undefined" ? Object : QueueManager,
    typeof ProcessorMetadataAccessor === "undefined" ? Object : ProcessorMetadataAccessor,
    typeof QueueEventBus === "undefined" ? Object : QueueEventBus,
...
function Processor(arg) {
  ...
  return (target) => {
    Injectable()(target);
    defineMetadata(PROCESSOR_METADATA, options, target);
  };
}
__name(Processor, "Processor");
function OnJobEvent(event, connection) {
  return (_target, _key, descriptor) => {
    updateMetadata(ON_JOB_EVENT_METADATA, [], (items) => [
      ...items,
      {
        event,
        connection
...
function Job(options) {
  ...
  return (target) => {
    ...
    defineMetadata(JOB_METADATA, options, target);
  };
}
__name(Job, "Job");
function InjectQueue(queue2, connection) {
  return Inject(getQueueToken(queue2, connection));
}
...
var queue = inject(QUEUE_MANAGER);
...
export { BaseConnection, BroadcastChannelConnection, IndexedDBConnection, InjectQueue, InjectQueueConnection, JOB_METADATA, Job, LocalStorageConnection, MaxAttemptsExceededError, MemoryConnection, NullConnection, ON_JOB_EVENT_METADATA, OnJobEvent, PROCESSOR_METADATA, Processor, ProcessorMetadataAccessor, ProcessorSubscribersLoader, QStashConnection, QueueDriverError, QueueError, QueueEventBus, QueueHandle, QueueManager, QueueModule, SyncConnection, TimeoutExceededError, Worker, WorkerHost, getQueueConnectionToken, getQueueToken, queue, useQueue, useQueueConnection, useQueueManager };
