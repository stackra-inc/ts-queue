...
import { Injectable, Inject, Optional, Global, Module } from '@stackra/ts-container';
import { Logger } from '@stackra/ts-logger';
import { QUEUE_CONFIG, EVENT_EMITTER, TAB_COORDINATOR, QUEUE_MANAGER, DEFAULT_QUEUE_CONNECTION_TOKEN, JobStatus, QUEUE_EVENTS } from '@stackra/contracts';
...
var QueueManager = class extends MultipleInstanceManager {
  /**
...
QueueManager = __decorateClass([
  Injectable(),
  __decorateParam(0, Inject(QUEUE_CONFIG))
], QueueManager);
...
QueueEventBus = __decorateClass([
  Injectable(),
  __decorateParam(0, Optional()),
  __decorateParam(0, Inject(EVENT_EMITTER))
], QueueEventBus);
...
ProcessorMetadataAccessor = __decorateClass([
  Injectable()
...
var Worker = class _Worker {
  constructor(config) {
     ...
     */
    this.logger = new Logger(_Worker.name);
...
var ProcessorSubscribersLoader = class {
  constructor(manager, metadataAccessor, eventBus, events) {
     ...
     */
    this.logger = new Logger(ProcessorSubscribersLoader.name);
    /**
...
ProcessorSubscribersLoader = __decorateClass([
  Injectable(),
  __decorateParam(3, Optional()),
  __decorateParam(3, Inject(EVENT_EMITTER))
...
MemoryConnector = __decorateClass([
  Injectable()
], MemoryConnector);
var SyncConnector = class {
  /**
  ...
  async push(jobName, data, options) {
    ...
    if (job.uniqueId) {
      const existing = blob.jobs.find(
        (j) => j.uniqueId === job.uniqueId && j.status !== JobStatus.Completed && j.status !== JobStatus.Failed
      );
...
var LocalStorageConnector = class {
  /**
...
LocalStorageConnector = __decorateClass([
  Injectable()
...
IndexedDBConnector = __decorateClass([
  Injectable()
], IndexedDBConnector);
...
var BroadcastChannelConnector = class {
  /**
...
BroadcastChannelConnector = __decorateClass([
  Injectable(),
  __decorateParam(0, Optional()),
  __decorateParam(0, Inject(TAB_COORDINATOR))
...
QStashConnector = __decorateClass([
  Injectable()
], QStashConnector);
...
QueueModule.logger = new Logger(QueueModule.name);
